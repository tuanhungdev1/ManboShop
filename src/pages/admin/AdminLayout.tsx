...
const AdminLayout = () => {
  ...
  return (
    <div className="h-screen overflow-hidden flex items-stretch bg-gray-100">
      {/* Sidebar */}
      <div className="bg-white w-[320px] border-r-[2px] py-12 px-6">
        {/* Logo */}
        <div className="flex items-center justify-center">
          <Link to="/admin" className="flex-shrink-0 scale-125">
            <svg
              width="94"
              height="23"
              viewBox="0 0 94 23"
          ...
          {AdminMenu.map((item) => {
            ...
            return (
              <Link to={item.path} key={item.id}>
                <li
                  className={cn(
                    `flex font-medium text-sm transition-all opacity-60 duration-200 py-4 rounded-md pl-10 gap-3 items-center`,
                    pathname === item.path && "bg-gray-100 opacity-100"
                  )}
                >
