...
const ProductList = ({ viewMode }: ProductListProps) => {
  ...
  const { data, isLoading, isFetching } = useGetProductsQuery({
    ...
    categoryId: filters.productTypes.length > 0 ? Number(filters.productTypes[0]) : undefined,
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className={`grid ${
        ...
        {data.data.map((product) => (
          <div 
            key={product.id}
            className={`${isFetching ? 'opacity-50' : 'opacity-100'} transition-opacity duration-300`}
          >
            <ProductCard 
              product={product} 
              viewMode={viewMode}
