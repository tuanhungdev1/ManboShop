...
import { useUpdateAddressMutation } from "@services/addressApi";
import { AddressDto, AddressForUpdateDto } from "@types-d/address";
...
export const addressForUpdateValidationSchema = yup.object({
  name: yup
    .string()
...
const AddressUpdateModel = ({
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<AddressForUpdateDto>({
    resolver: yupResolver(addressForUpdateValidationSchema),
    defaultValues: {
  ...
  const onSubmit: SubmitHandler<AddressForUpdateDto> = async (data) => {
    try {
      await updateAddress({
        id: initialData.id,
        addressDto: data,
  ...
  return (
    <Modal open={open} onClose={onClose} aria-labelledby="update-address-modal">
      <Box sx={modalStyle}>
        <Box
          display="flex"
          ...
          mb={3}
        >
          <Typography variant="h6">Cập Nhật Địa Chỉ</Typography>
